...
export default function StarOrb({ position = [0, 0, 0], media = "" }) {
  ...
  useFrame(() => {
    if (orbRef.current) {
      ...
      // lerp expects THREE.Vector3
      orbRef.current.scale.lerp(new THREE.Vector3(target, target, target), 0.1);
    }
  });
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial color={hovered ? "#ff69b4" : "#ffffff"} emissive={hovered ? "#ff69b4" : "#222"} metalness={0.3} roughness={0.2} />
